...
if [ "${1:-}" = "--install-cron" ]; then
    CRON_CMD="0 3 * * * cd $PROJECT_DIR && ./scripts/backup.sh >> /var/log/ptx-backup.log 2>&1"
    ...
    ( crontab -l 2>/dev/null || true ) | ( grep -v "ptx-backup" || true ) | { cat; echo "$CRON_CMD"; } | crontab -
    echo "Installed daily backup cron job (3:00 AM)."
    echo "Logs: /var/log/ptx-backup.log"
    exit 0
fi
